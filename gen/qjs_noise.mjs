// gen/qjs_noise.mjs - QuickJS C++ Binding Emitter for bro.noise / FastNoise
// Generates out/qjs/noise.cpp (drop-in replacement for bro/third_party/brokit/src/api/noise.cpp)

/**
 * Emits the C++ translation unit for FastNoise QuickJS bindings.
 * @param {Array<Object>} astList - List of parsed IDL AST file nodes
 * @returns {string} Emitted C++ source code
 */
export function emitNoiseBindings(astList) {
  let fastNoiseInterface = null;

  for (const fileAst of astList) {
    for (const def of fileAst.definitions) {
      if (def.type === 'Interface' && def.name === 'FastNoise') {
        fastNoiseInterface = def;
        break;
      }
    }
    if (fastNoiseInterface) break;
  }

  if (!fastNoiseInterface) {
    throw new Error("Could not find 'FastNoise' interface definition in IDL AST");
  }
