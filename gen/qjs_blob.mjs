// gen/qjs_blob.mjs - QuickJS C++ Binding Emitter for Blob & File
// Generates out/qjs/blob.cpp (drop-in replacement for bro/third_party/brokit/src/api/blob.cpp)

/**
 * Emits the C++ translation unit for Blob and File QuickJS bindings.
 * @param {Array<Object>} astList - List of parsed IDL AST file nodes
 * @returns {string} Emitted C++ source code
 */
export function emitBlobBindings(astList) {
  let blobInterface = null;
  let fileInterface = null;

  for (const fileAst of astList) {
    for (const def of fileAst.definitions) {
      if (def.type === 'Interface' && def.name === 'Blob') {
        blobInterface = def;
      }
      if (def.type === 'Interface' && def.name === 'File') {
        fileInterface = def;
      }
    }
  }
  ...
  if (!blobInterface) {
    throw new Error("Could not find 'Blob' interface definition in IDL AST");
  }
  if (!fileInterface) {
    throw new Error("Could not find 'File' interface definition in IDL AST");
  }

  const out = [];

  out.push(`#include "api/api.h"
